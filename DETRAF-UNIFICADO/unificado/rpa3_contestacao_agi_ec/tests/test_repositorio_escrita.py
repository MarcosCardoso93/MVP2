...
def test_atualizar_despesa_atualiza_os_seis_campos_sem_criar_linha(repo_tabelas):
    ...
    # `vb_contestacao` não existe no banco real (Q24), e o fixture o espelha —
    # por isso ela aparece em `colunas_ignoradas` e as outras seis gravam.
    assert resumo == {
        "atualizadas": 1,
        "ausentes": [],
...
def test_atualizar_despesa_lote_vazio_nao_falha(repo_tabelas):
    ...
    assert resumo == {"atualizadas": 0, "ausentes": [], "colunas_ignoradas": []}


def test_coluna_ausente_nao_derruba_o_update_das_demais(repo_tabelas, caplog):
    """
    Q24 — `vb_contestacao` não existe no banco, e o resto da HU-19 grava assim mesmo.

    🔴 O modo de falha que isto previne: `atualizar_dados` monta **um único**
    `UPDATE ... SET a=:a, b=:b ...`. Uma coluna inexistente derruba a instrução
    inteira, e com ela os seis campos que existem — para **todas** as operadoras
    do lote, não só a linha problemática. O robô reportaria erro e não teria
    escrito nada.

    O aviso é por LOTE, não por linha: o laço roda por operadora, e um aviso por
    linha soterraria o sinal num log de execução real.
    """
    df_contest = pd.DataFrame(
        [
            _linha_contest("021", "011", "202507"),
            _linha_contest("021", "012", "202507", remuneracao="VU-M"),
        ]
    )
    linhas = ec.preparar_atualizacoes_despesa_contestacao(df_contest, "202507")

    assert "vb_contestacao" in linhas.columns, "o preparo continua calculando o ¶942"

    with caplog.at_level(logging.WARNING):
        resumo = repo_tabelas.atualizar_despesa_contestacao(linhas)

    assert resumo["atualizadas"] == 2
    assert resumo["colunas_ignoradas"] == ["vb_contestacao"]

    avisos = [
        registro
        for registro in caplog.records
        if "vb_contestacao" in registro.getMessage()
    ]
    assert len(avisos) == 1, "um aviso por lote, não por linha"

    # E os seis campos da V2 chegaram ao banco.
    alvo = repo_tabelas.tbl_contestacao
    linha = alvo[alvo["id"] == 1].iloc[0]
    assert linha["minutos_operadora"] == pytest.approx(777.0)
    assert linha["vb_operadora"] == pytest.approx(-42.0)
    assert linha["vb_variacao_perc"] == pytest.approx(100.0)
        ...
        [
            {
                ...
                "referencia": "202507",
                "trafego": "202507",
                # Payload do banco -> plural. Ver `tabelas.COL_CONTESTACAO_REMUNERACOES`.
                "remuneracoes":"TU-RL",
