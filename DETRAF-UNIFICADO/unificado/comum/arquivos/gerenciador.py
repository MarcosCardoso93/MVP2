...
def separar_e_salvar_por_mascara(
    df: pd.DataFrame,
    mascara_validas: pd.Series,
    caminho_arquivo: Path,
    sufixo_erro: str = "_ERRO",
) -> Tuple[pd.DataFrame, pd.DataFrame, Path | None]:
    """
    Separa `df` em linhas válidas/inválidas segundo `mascara_validas` (True =
    válida). Se houver linhas inválidas, salva-as em um novo arquivo
    `{nome}{sufixo_erro}.{ext}` (via `salvar_dados`) e retorna esse caminho;
    se não houver, não salva nada e retorna (df, DataFrame vazio, None).

    Não aplica nenhuma lógica de totalização — quem chamar decide o que
    fazer com as linhas válidas (ex.: re-mesclar linhas de totalização antes
    de sobrescrever o arquivo original).
    """
    df_validas = df[mascara_validas]
    df_invalidas = df[~mascara_validas]

    if df_invalidas.empty:
        return df_validas, df_invalidas, None

    caminho_erro = caminho_arquivo.with_name(
        f"{caminho_arquivo.stem}{sufixo_erro}{caminho_arquivo.suffix}"
    )
    salvar_dados(df_invalidas, caminho_erro)

    return df_validas, df_invalidas, caminho_erro
...
# --- origem: Projeto 3 (exclusiva daquele projeto)
...
# --- origem: Projeto 4 (exclusiva daquele projeto)
def salvar_planilhas(
    abas: Dict[str, pd.DataFrame],
    caminho_salvamento: Path,
    incluir_cabecalho: bool = False,
) -> Path:
    """
    Salva múltiplos DataFrames como abas separadas de um único arquivo Excel.
    ...
    Args:
        abas: Mapeamento ``{nome_da_aba: DataFrame}``, na ordem em que devem
            aparecer no arquivo.
        caminho_salvamento (Path): Caminho completo de destino (deve ter
            extensão Excel, ex.: `.xlsx`).
        incluir_cabecalho (bool): Se False, salva cada aba sem a linha de
            nomes de coluna.

    Returns:
        Path: O caminho onde o arquivo foi salvo.

    Raises:
        ValueError: Se `abas` estiver vazio.
        IOError: Falha ao gravar o arquivo.
    """
    if not abas:
        raise ValueError("É necessário informar ao menos uma aba para salvar.")

    _validar_extensao(caminho_salvamento.suffix)

    try:
        caminho_salvamento.parent.mkdir(parents=True, exist_ok=True)

        with pd.ExcelWriter(caminho_salvamento, engine="openpyxl") as escritor:
