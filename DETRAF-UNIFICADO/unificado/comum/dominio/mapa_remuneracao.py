"""Resolver de remuneração a partir do descritor — base comum.

**Promovido de `rpa3/src/services/` para `comum/dominio/` em 2026-08-06**, ao
corrigir o defeito **A4**.

## Por que a promoção

Havia **duas** fontes de remuneração no repositório, com vocabulários diferentes:
o RPA 2 gravava `remuneracao` pela regra fixa do Épico 2
(`classificar_descritor_remuneracao`, cinco resultados) e o RPA 3 lia pelo
catálogo D-5 (21 finais distintos). Como essa coluna é **parte da chave** do
sinal do analista, e a comparação é igualdade exata de string, as duas pontas só
casavam nos dois valores em que os vocabulários coincidem — `TU-RL` e `VU-M`.
Para todo outro descritor o RPA 3 não encontrava o sinal, e o sintoma era
indistinguível de "o analista não sinalizou".

A correção é ter **uma fonte só**: quem grava e quem lê usam este módulo.

Os critérios de compartilhamento (C1-C4) já se cumpriam antes — dois consumidores
reais em RPAs diferentes, regra fechada, independente de RPA. O que travava era o
**bloqueio B-D21**: o schema da tabela não estava confirmado. Confirmado em
2026-08-06 (cinco colunas), a promoção deixou de ter impedimento.

⚠️ A regra fixa do Épico 2 **continua existindo e continua certa** — para outra
coisa. Ela devolve o vocabulário de `tbl_detraf_tarifas.tipo_remuneracao`
(`TU-RIU1`, `TU-RIU2`, `TU-RL`, `VU-M`), que é o da **tarifa regulada**, com o RIU
dividido em dois porque a tarifa difere. É o que a validação de tarifa precisa.
Ver `comum/dominio/classificadores.py`.
...
Colunas esperadas — **confirmadas em 2026-08-06** (Q22): ``id``,
