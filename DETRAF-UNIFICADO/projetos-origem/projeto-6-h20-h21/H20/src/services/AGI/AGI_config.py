...
class AGI_CONFIG():
    ...
    def _Janela_salvar(self,diretorio,nome_janela):
        ...
        # mesmo cuidado bilingue do dialogo principal - "Confirm Save As" (EN) / "Confirmar Salvar como" (PT-BR)
    ...
    def Baixar_Remessa(self):
        ...
        while True:
            ...
            if sub_menu != "Não encontrou a imagem":
                    break
        # O idioma do dialogo muda conforme a VM (ja vimos em ingles e em portugues) - regex
        # aceita as duas variantes em vez de depender do idioma configurado na maquina.
        nome_janela = rf"(Select location for download by|Selecionar local para download de) {AGI_JANELA_HOST}"
        self._Janela_salvar(diretorio=os.path.join(DIRETORIO_REMESSA_BAIXADA,"remessa_baixada.csv"),nome_janela=nome_janela)
        # self.AGI_CONFIG._wait_appear(img_bnt_ok_export,timeout=300)

        caminho_csv = os.path.join(DIRETORIO_REMESSA_BAIXADA,"remessa_baixada.csv")
        ...
        # Logo apos o download, o arquivo pode ainda estar retido (antivirus escaneando o
        # download novo, ou o proprio processo que salvou ainda nao liberou o handle) -
        # isso derruba o open("w") com PermissionError/Access denied de forma transitoria.
        # Tenta de novo com pequenas esperas antes de desistir.
        for tentativa in range(1, 6):
            try:
                os.chmod(caminho_csv, stat.S_IWRITE)
                with open(caminho_csv, "w", encoding="utf-8") as f:
                    f.writelines(linhas_corrigidas)
                break
            except PermissionError as e:
                print(f"[Baixar_Remessa] arquivo ainda bloqueado, tentativa {tentativa}/5: {e}")
                time.sleep(2)
        else:
            raise PermissionError(f"nao foi possivel reescrever '{caminho_csv}' apos 5 tentativas")
