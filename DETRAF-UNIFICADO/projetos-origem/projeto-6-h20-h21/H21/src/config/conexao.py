...
class Banco:
    ...
    def log(self,processo,status,descricao):
        ...
        log = {
        "rpa": "Detraf Despesa - HU-21 (Retificacao/Recuperacao)",
        "processo": processo,
        "status": status,
        "descricao": descricao,
        "data_log": data_log
        ...
        # TODO: nome da tabela sugerido (nao confirmado com o solicitante) - so as tabelas de
        # HU-15/HU-17-18 foram citadas explicitamente no To Be MVP2 (paragrafo 5.4.5.9).
        result = self._inserir_banco("tbl_rpa_log_detraf_despesa_retificacao",log, config=self.config_log)
        return result
    ...
    def marcar_carga_agi(self, id_linha, status="carregado"):
        """
        Marca a linha de tbl_rpa_log_detraf_despesa_contestacao como ja processada no
        AGI (HU-21), para nao ser retificada de novo no proximo ciclo.
        """
        return self._atualizar_banco(
