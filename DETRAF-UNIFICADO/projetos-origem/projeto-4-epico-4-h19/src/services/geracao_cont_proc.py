...
- T-101: `DURACAO`/`VLR_BRUTO` (negativos, a partir da diferença já calculada pelo
  `Contest`), `REMUNERACAO_FIXA` (já resolvida pelo `Contest` via D-5) e `ID_MODALIDADE`
    ...
    chaves = pd.DataFrame(
        {
            ...
            "remuneracao": candidatas["remuneracao"].astype(str),
        }
    )

    # Uma consulta por combinação única, não por linha.
    sinal_por_chave = {
    ...
    resultado = pd.DataFrame(
        {
            ...
            COL_VLR_BRUTO: -contestadas["vb_diferenca"],
