...
def nome_base_contestacao(operadora: str, mes: str) -> str:
    """
    Nome-base do arquivo Base_Contestação (dependência interna).

    ``Base_Contestação_{operadora}_{mes}``
    """

    op = normalizar_operadora(operadora)
    return f"{const.PREFIXO_BASE_CONTESTACAO}_{op}_{mes}"


def nome_modelo_base_contestacao(operadora: str, mes: str) -> str:
    """Nome-base do modelo ``_M`` do Base_Contestação: ``..._{mes}_M`` (HU-14)."""

    return f"{nome_base_contestacao(operadora, mes)}_{const.SUFIXO_MODELO_BASE}"
