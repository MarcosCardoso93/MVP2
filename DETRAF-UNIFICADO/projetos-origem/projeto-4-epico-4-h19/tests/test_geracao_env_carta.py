...
def test_pasta_inexistente_retorna_1(tmp_path: Path):
    assert gec.obter_proximo_numero_carta(tmp_path / "nao_existe") == 1


def test_pasta_vazia_retorna_1(tmp_path: Path):
    pasta = tmp_path / "CT" / "2025"
    pasta.mkdir(parents=True)
    assert gec.obter_proximo_numero_carta(pasta) == 1
