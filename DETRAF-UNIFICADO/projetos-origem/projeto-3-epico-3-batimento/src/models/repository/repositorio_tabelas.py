...
class RepositorioTabelas:
    ...
    def __init__(self) -> None:
        ...
        self.tbl_detraf_mapeamento_descritores: pd.DataFrame = self.cache.obter_tabela(
    ...
    def salvar_dados_tabela_contestacao(self, df_contestacao: pd.DataFrame) -> None:
        """
        Salva os dados de análise de contestação (H9/H10) na tabela
        'tbl_rpa_log_detraf_despesa_contestacao' do banco.

        Args:
            df_contestacao (pd.DataFrame): DataFrame já no formato de colunas
                da tabela de destino (ver CriacaoArquivoContestacao).
        """
    ...
    def validar_coluna_eot_df(self, df: pd.DataFrame, indice_coluna: int) -> bool:
        ...
        # 1. Sanitiza o conjunto de EOTs válidos do cache usando o método estático da classe
        eots_cache_brutos = self.tbl_anexo5_processado["EOT"].astype(str)
        eots_cache_tratados = eots_cache_brutos.apply(self._tratar_eot)

        # Transforma em um 'set' para busca ultraveloz O(1) em memória
        eots_validos_set: set[str] = set(eots_cache_tratados.unique())

        # 2. Extrai e trata a coluna alvo do DataFrame que veio do arquivo (ex: coluna devedora)
        coluna_alvo_bruta = df.iloc[:, indice_coluna].astype(str)
        coluna_alvo_tratada = coluna_alvo_bruta.apply(self._tratar_eot)

        # 3. Validação em lote: Verifica se todos (.all()) os registros estão contidos no conjunto válido
        valido = bool(coluna_alvo_tratada.isin(eots_validos_set).all())
    ...
    def validar_eots_por_nomes_fantasia(
        ...
        # 1. Busca e isola os EOTs válidos no banco de dados (tabela cache)
        ...
        # Aplica o _tratar_eot nos EOTs do banco e guarda em um set O(1)
        eots_validos_banco = set(
            sub_tabela_banco["EOT"].astype(str).apply(self._tratar_eot).unique()
        )

        # 2. Extrai e trata a coluna alvo do DataFrame do arquivo usando o mesmo _tratar_eot
        coluna_df_bruta = df.iloc[:, indice_coluna].astype(str)
        coluna_df_tratada = coluna_df_bruta.apply(self._tratar_eot)

        # 3. Validação em lote: Verifica se todos os EOTs tratados do DF existem no set do banco
        mascara_validos = coluna_df_tratada.isin(eots_validos_banco)
    ...
    def obter_tipo_produto_por_poi(self, poi: str) -> Optional[str]:
        """
        Busca o Tipo de Produto / Remuneração na tbl_detraf_mapeamento_descritores
        usando o último caractere do descritor (código POI) como chave, seguindo
        o mesmo padrão de classificação usado no Épico 2 (classificadores.py):
        o descritor é mapeado pelo caractere final (coluna FINAL_DO_DESCRITOR).

        Quando mais de uma linha compartilha o mesmo caractere final (ex.: "T"
        mapeia tanto para TU-RIU quanto para VU-T na base atual), a primeira
        ocorrência é usada e um aviso é registrado, pois a tabela não permite
        desambiguar apenas pelo caractere final.
        """
        poi_tratado = str(poi).strip().upper()

        if not poi_tratado:
            return None

        caractere_final = poi_tratado[-1]

        resultado = self.tbl_detraf_mapeamento_descritores[
            self.tbl_detraf_mapeamento_descritores["FINAL_DO_DESCRITOR"]
            .astype(str)
            .str.strip()
            .str.upper()
            == caractere_final
        ...
        if resultado.empty:
            logger.info(
                f"Descritor não encontrado em tbl_detraf_mapeamento_descritores "
                f"para o POI [{poi_tratado}] (caractere final [{caractere_final}])"
            )
            return None

        if len(resultado) > 1:
            logger.warning(
                f"Caractere final [{caractere_final}] (POI [{poi_tratado}]) é ambíguo em "
                f"tbl_detraf_mapeamento_descritores: {resultado['REMUNERACAO_FIXA'].tolist()}. "
                f"Usando a primeira ocorrência."
            )

        return resultado.iloc[0]["REMUNERACAO_FIXA"]
